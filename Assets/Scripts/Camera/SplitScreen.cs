...
public class SplitScreen : MonoBehaviour
{
    ...
    /*References to deal when there is only one Player*/
    ...
    private UniversalAdditionalCameraData camera1Data;
    private UniversalAdditionalCameraData camera2Data;
    ...
    void Start()
    {
        ...
        camera1 = Camera.main;
        camera1Data = camera1.GetUniversalAdditionalCameraData();
        var camera2Obj = new GameObject("Camera 2");
        camera2 = camera2Obj.AddComponent<Camera>();

        // Ensure the second camera renders before the first one
        camera2.depth = camera1.depth - 1;
        var camera2Data = camera2.GetUniversalAdditionalCameraData();
        camera2Data.renderType = CameraRenderType.Base;
        camera2Data.cameraStack.Add(camera1);
        ...
        camera2.cullingMask = ~(1 << LayerMask.NameToLayer("TransparentFX"));
    ...
    void LateUpdate()
    {
        if (!mergeBehaviour.getIsMerged())
        {
            ...
            if (distance > splitDistance)
            {
                ...
                ActivatesSplitCamera();
                MoveCamera2(midPoint2);

            }
            else
            {
                DeactivateSplitCamera();
            }
        ...
        {
            Vector3 midPoint = new Vector3(bigPlayer.position.x, bigPlayer.position.y, bigPlayer.position.z);
    ...
    private void MoveCamera2(Vector3 midPoint2)
    {
    ...
    private void DeactivateSplitCamera()
    {
        ...
        camera1Data.renderType = CameraRenderType.Base;
    }

    private void ActivatesSplitCamera()
    {
        ...
        camera1Data.renderType = CameraRenderType.Overlay;
